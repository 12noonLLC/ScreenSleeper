...
[![Build and Release](https://github.com/12noonLLC/ScreenSleeper/actions/workflows/build.yml/badge.svg)](https://github.com/12noonLLC/ScreenSleeper/actions/workflows/build.yml)
...
1. If `--delay` value is greater than zero, wait this long.
1. If `--standby` value is greater than zero, set monitor(s) to standby and wait this long before turning them off.
1. If `--lock` is specified, lock Windows.
1. If `--hibernate` is specified, hibernate the computer.
1. If `--sleep` is specified, put the computer to sleep.
1. If neither is specified, turn off monitor(s).

*ScreenSleeper* always waits at least three seconds before turning off the
monitors (step 2) so that the user has time to stop using the keyboard and mouse.
(Otherwise, they might unintentionally wake up the computer.)

Hibernate takes precedence over sleep, so if both `--sleep` and `--hibernate` are specified, the computer will hibernate.
...
--delay \<seconds\> | Wait before entering standby (if --standby is specified) or off (Default = 3)
